...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.ipaddress.title_singular') }}
    </div>

    <div class="card-body">
        <form action="{{ route('admin.ipaddress.update', [$ipaddress->id]) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="form-group {{ $errors->has('serverid') ? 'has-error' : '' }}">
                ...
                @foreach($server as $id => $server)
                    <option value="{{ $id }}" {{ (in_array($id, old('server', [])) || isset($ipaddress) && $ipaddress->servers()->pluck('id')->contains($id)) ? 'selected' : '' }}>{{ $server }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group {{ $errors->has('countriesid') ? 'has-error' : '' }}">
                ...
                @foreach($countries as $id => $countries)
                    <option value="{{ $id }}" {{ (in_array($id, old('countries', [])) || isset($ipaddress) && $ipaddress->countries()->pluck('id')->contains($id)) ? 'selected' : '' }}>{{ $countries }}</option>
                    @endforeach
                </select>
